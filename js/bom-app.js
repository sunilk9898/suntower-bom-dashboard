...
function openN(i) { const n = notices[i]; if (!n) return; document.getElementById('mTitle').textContent = n.title || 'Notice'; const b = document.getElementById('mBody'); if (n.fileUrl) { const url = n.fileUrl; const isGDrive = url.includes('drive.google.com'); const isPdf = n.fileType === 'pdf' || url.includes('.pdf'); const isImage = /\.(jpg|jpeg|png|gif|webp|bmp|svg)(\?|$)/i.test(url); if (isGDrive) { const fileIdMatch = url.match(/\/d\/([a-zA-Z0-9_-]+)/); const previewUrl = fileIdMatch ? `https://drive.google.com/file/d/${fileIdMatch[1]}/preview` : url; b.innerHTML = `<iframe src="${previewUrl}" style="width:100%;min-height:600px;border:none" allow="autoplay"></iframe><div style="text-align:center;margin-top:12px"><a href="${url}" target="_blank" class="btn btn-primary btn-sm" style="text-decoration:none">Open in Google Drive</a></div>`; } else if (isPdf) { b.innerHTML = `<iframe src="${url}" style="width:100%;min-height:600px;border:none"></iframe>`; } else if (isImage) { b.innerHTML = `<img src="${url}" style="max-width:100%">`; } else { b.innerHTML = `<div style="padding:20px;text-align:center"><div style="font-size:3rem;margin-bottom:12px">📎</div><p style="margin-bottom:12px">${esc(n.title || 'Document')}</p><a href="${url}" target="_blank" class="btn btn-primary btn-sm" style="text-decoration:none">Download / View File</a></div>`; } } else b.innerHTML = `<div style="padding:20px"><p><strong>Date:</strong> ${n.date || ''}</p><p><strong>Category:</strong> ${n.category || ''}</p><p style="margin-top:15px">${n.summary || ''}</p></div>`; document.getElementById('noticeModal').classList.remove('hidden'); }
