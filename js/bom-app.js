...
// ===== PROJECT TRACKER =====
// Derive project lookup maps from dynamic commDefs
function _getCommMap(key) { const m = {}; (typeof commDefs !== 'undefined' ? commDefs : DEFAULT_COMM_DEFS).forEach(d => m[d.c] = d[key]); return m; }
function getCommNames() { return _getCommMap('n'); }
function getCommColorMap() { return _getCommMap('bg'); }
function getCommIcons() { return _getCommMap('icon'); }
function getCommSOP() { return _getCommMap('sop'); }
...
// ===== DOCUMENTS =====
async function loadDocuments() { const el = document.getElementById('documentsList'); if (!el) return; try { if (!notices || notices.length === 0) { notices = JSON.parse(localStorage.getItem('st_notices') || '[]'); } const catFilter = document.getElementById('docCategoryFilter')?.value || ''; const searchQuery = document.getElementById('docSearch')?.value?.toLowerCase() || ''; let docs = notices.filter(n => n.fileUrl); if (catFilter) docs = docs.filter(d => d.category === catFilter); if (searchQuery) docs = docs.filter(d => (d.title || '').toLowerCase().includes(searchQuery) || (d.summary || '').toLowerCase().includes(searchQuery)); if (docs.length === 0) { el.innerHTML = '<div class="kpi-empty" style="grid-column:1/-1">No documents found</div>'; return; } el.innerHTML = docs.map((d, i) => { const catColors = {'General':'#1a237e','Maintenance':'#e65100','Security':'#b71c1c','Meeting':'#1565c0','Financial':'#2e7d32','Event':'#4527a0'}; const color = catColors[d.category] || '#666'; const ni = notices.indexOf(d); const isPdf = d.fileType === 'pdf' || d.fileUrl.includes('.pdf'); const isGDrive = d.fileUrl.includes('drive.google.com'); const docIcon = isPdf ? '📄' : isGDrive ? '📁' : /\.(jpg|jpeg|png|gif|webp)/i.test(d.fileUrl) ? '🖼️' : '📎'; return `<div class="card" style="cursor:pointer;border-top:3px solid ${color};margin:0;position:relative" onclick="openN(${ni})"><div style="font-size:2rem;margin-bottom:8px">${docIcon}</div>${isAdmin ? `<button onclick="event.stopPropagation();deleteDocument(${ni})" style="position:absolute;top:8px;right:8px;background:none;border:none;color:#bbb;font-size:1.2rem;cursor:pointer;padding:2px 6px" title="Delete document">&times;</button>` : ''}<h4 style="font-size:0.92rem;margin-bottom:4px">${esc(d.title || 'Document')}</h4><div style="font-size:0.78rem;color:var(--text-light);margin-bottom:8px">${esc(d.summary || '').substring(0, 80)}${(d.summary || '').length > 80 ? '...' : ''}</div><div style="display:flex;gap:6px;align-items:center"><span class="badge" style="background:${color};font-size:0.65rem">${esc(d.category || 'General')}</span><span style="font-size:0.72rem;color:#999">${esc(d.date || '')}</span></div></div>`; }).join(''); } catch(e) { el.innerHTML = '<div class="kpi-empty" style="grid-column:1/-1">Error loading documents</div>'; } }
...
function openN(i) { const n = notices[i]; if (!n) return; document.getElementById('mTitle').textContent = n.title || 'Notice'; const b = document.getElementById('mBody'); let content = ''; const delBtn = isAdmin ? `<div style="text-align:center;margin-top:12px"><button class="btn btn-danger btn-sm" onclick="deleteDocument(${i});closeModal()">Delete Document</button></div>` : '';
  if (n.fileUrl) { const url = n.fileUrl; const isGDrive = url.includes('drive.google.com'); const isPdf = n.fileType === 'pdf' || url.includes('.pdf'); const isImage = /\.(jpg|jpeg|png|gif|webp|bmp|svg)(\?|$)/i.test(url); if (isGDrive) { const fileIdMatch = url.match(/\/d\/([a-zA-Z0-9_-]+)/); const previewUrl = fileIdMatch ? `https://drive.google.com/file/d/${fileIdMatch[1]}/preview` : url; content = `<iframe src="${previewUrl}" style="width:100%;min-height:600px;border:none" allow="autoplay"></iframe><div style="text-align:center;margin-top:12px"><a href="${url}" target="_blank" class="btn btn-primary btn-sm" style="text-decoration:none">Open in Google Drive</a></div>`; } else if (isPdf) { content = `<iframe src="${url}" style="width:100%;min-height:600px;border:none"></iframe>`; } else if (isImage) { content = `<img src="${url}" style="max-width:100%">`; } else { content = `<div style="padding:20px;text-align:center"><div style="font-size:3rem;margin-bottom:12px">📎</div><p style="margin-bottom:12px">${esc(n.title || 'Document')}</p><a href="${url}" target="_blank" class="btn btn-primary btn-sm" style="text-decoration:none">Download / View File</a></div>`; }
  } else { content = `<div style="padding:20px"><p><strong>Date:</strong> ${n.date || ''}</p><p><strong>Category:</strong> ${n.category || ''}</p><p style="margin-top:15px">${n.summary || ''}</p></div>`; }
  b.innerHTML = content + delBtn; document.getElementById('noticeModal').classList.remove('hidden'); }
function closeModal() { document.getElementById('noticeModal').classList.add('hidden'); }

async function deleteDocument(i) {
  const n = notices[i];
  if (!n) return;
  if (!confirm(`Delete "${n.title || 'this document'}"?`)) return;
  // Try to delete from Google Drive if connected and it's a GDrive file
  if (_gdriveToken && n.fileUrl && n.fileUrl.includes('drive.google.com')) {
    const fileIdMatch = n.fileUrl.match(/\/d\/([a-zA-Z0-9_-]+)/);
    if (fileIdMatch) {
      try {
        await fetch(`https://www.googleapis.com/drive/v3/files/${fileIdMatch[1]}`, {
          method: 'DELETE', headers: { 'Authorization': 'Bearer ' + _gdriveToken }
        });
      } catch(e) { console.warn('Could not delete from Google Drive:', e); }
    }
  }
  // Remove from notices array and localStorage
  notices.splice(i, 1);
  localStorage.setItem('st_notices', JSON.stringify(notices));
  supaSync('st_notices');
